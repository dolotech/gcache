...
func (q *queryHandle) BySearch(scope *easyScope) (primarys []interface{}, e error) {
	...
	if jsearch != nil && (jsearch.Timeout+int64(scope.opt.Expires)) > time.Now().Unix() {
...
func (q *queryHandle) ByCount(scope *easyScope) (count int, e error) {
	...
	if jsearch != nil && (jsearch.Timeout+int64(scope.opt.Expires)) > time.Now().Unix() && len(jsearch.Primarys) > 0 {
...
// getSearchPrimarys
func (q *queryHandle) getSearchPrimarys(table string, key string, field string) (jsearch *JsonSearch, e error) {
	searchKey := q.handle.JoinSearchKey(table, key)
...
// getSearchPrimarys
func (q *queryHandle) getSearchCount(table string, key string, field string) (jsearch *JsonSearch, e error) {
	field = q.handle.JoinCountSecondKey(field)
	serachKey := q.handle.JoinSearchKey(table, key)
